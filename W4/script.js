...
btnYes.addEventListener('click', () => {
  ...
  confirmation.textContent = 'Yipee, party time!';
...
btnNo.addEventListener('click', () => {
  ...
  regret.textContent = `We are sorry ${getName()} Better luck next time!`;
...
const updateConfirmation = () => {
  ...
  guestline = `bringing ${guests} guest.`;
  } else if (guests > 1){
  guestline = `bringing ${guests} guests.`;}
  
  confirmation.textContent = `${getName()} is coming and is ${guestline}`;
